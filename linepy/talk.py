...
class Talk(object):
    ...
    @loggedIn
    def sendMessageMusic2(self, to, title=None, subText=None, url=None, iconurl=None, contentMetadata={}):
        """
        a : Android
        i : Ios
        """
        self.profile = self.getProfile()
        self.profile2 = self.getProfile()
        self.userTicket = self.generateUserTicket()
        title = title if title else self.profile.displayName
        subText = subText if subText else self.profile2.statusMessage
